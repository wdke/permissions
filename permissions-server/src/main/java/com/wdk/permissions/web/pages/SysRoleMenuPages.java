...
//页面控制类
@Controller
@RequestMapping("pages/sys/role/menu")
public class SysRoleMenuPages {

	//定义生成logger对象
	private static Logger logger=LoggerFactory.getLogger(SysRoleMenuPages.class);
	...
	*/
	...
	public String index(SysRoleMenuArgs sysRoleMenu, PageParam pageParam, Model model){

		//打印方法开始参数
		logger.info("index start。param:【sysRoleMenu->{},pageParam->{}】", JSON.toJSONString(sysRoleMenu), JSON.toJSONString(pageParam));

		//纪录开始时间
		long startTimes = System.currentTimeMillis();

		//调用业务逻辑
		PageInfo<SysRoleMenu> result = sysRoleMenuService.pageInfo(sysRoleMenu, pageParam);
	...
	*/
	...
	public String insert(SysRoleMenuArgs sysRoleMenu){

		//打印方法开始参数
		logger.info("insert start。param:【sysRoleMenu->{}】", JSON.toJSONString(sysRoleMenu));

		//纪录开始时间
		long startTimes = System.currentTimeMillis();

		//调用业务逻辑
		Integer result = sysRoleMenuService.insertSelective(sysRoleMenu);

		//打印方法开始参数
		logger.info("insert end。result:【data->{},times->{}】", JSON.toJSONString(result), TimeUtils.format(System.currentTimeMillis() - startTimes));
		return "redirect:/sys/role/menu";
	}
	...
	*/
	...
	public String updatePage(Integer id, Model model){
		...
		//根据主键查询数据
		SysRoleMenu result = sysRoleMenuService.selectByPrimaryKey(id);
	...
	*/
	...
	public String update(SysRoleMenuArgs sysRoleMenu){

		//打印方法开始参数
		logger.info("update start。param:【sysRoleMenu->{}】", JSON.toJSONString(sysRoleMenu));

		//纪录开始时间
		long startTimes = System.currentTimeMillis();

		//调用业务逻辑
		Integer result = sysRoleMenuService.updateSelectiveByPrimaryKey(sysRoleMenu);
